...
class HttpViewController extends MonitorController {
	...
	public function index() {
		...
		$alarmsnum = 0; // 警报数量
		$i = 0;
		if ($alarmslist [0] ['type'] == 0) {
			$alarmsnum ++;
			$i ++;
			$operator_type = $alarmslist [0] ['operator_type'];
			$threshold = $alarmslist [0] ['threshold'];
			$alarm_itemid = $alarmslist [0] ['index_id'];
			...
			$ltime = abs ( time() - $alarmslist [0] ['times'] );
			$temp ['stime'] = date ( "Y-m-d H:i:s", $alarmslist [0] ['times'] ); // 告警时间
			$temp ['status'] = "<span class=\"label label-warning\">故障</span>"; // 当前状态
			$temp ['ltime'] = changeTimeType ( $ltime ); // 持续时间
			$alarmdatas [] = $temp;
		}
		for($i; $i < (count ( $alarmslist ) - 1); $i = $i + 2) {
			$temp = array ();
			$i2 = $i + 1;
			
			$alarm_itemid = $alarmslist [$i2] ['index_id'];
			$operator_type = $alarmslist [$i2] ['operator_type'];
			$threshold = $alarmslist [$i2] ['threshold'];
			...
			$temp ['stime'] = date ( "Y-m-d H:i:s", $alarmslist [$i2] ['times'] ); // 告警时间
